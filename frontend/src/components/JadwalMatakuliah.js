// frontend/src/components/JadwalMatakuliah.js
...
const JadwalMatakuliah = () => {
  ...
  const [selectedSemester, setSelectedSemester] = useState(""); // Untuk filter semester
  ...
  const refreshToken = async () => {
    try {
      ...
      setIsAdmin(!!decoded.id_admin); // Cek apakah admin
    } catch (error) {
      navigate("/");
    }
  };
  ...
  const getMatakuliah = async () => {
    try {
      ...
      console.log(error);
      setMsg("Gagal memuat jadwal pelajaran");
    }
  };
  ...
  // Filter matakuliah berdasarkan semester
  const filteredMatakuliah = selectedSemester
    ? matakuliah.filter((item) => item.semester === parseInt(selectedSemester))
    : matakuliah;

  return (
    <div className="container mt-5">
      <Navbar />
      <h1 className="title">Jadwal Mata Kuliah</h1>
